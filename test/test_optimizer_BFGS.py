...
class TestOptimization(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.m_sand = 5 # sand material
        self.m_contact = 6 # contact material
    ...
    def minimisation_test_recipe(self, m):
        ...
        opt = optimizers.QuasiNewtonBFGS(grad, alpha_max=1, alpha_init=0.00001)
        sol_k = opt.optimise(k, max_iter=100)
...
class TestOptimizationNoisy(unittest.TestCase):
    def setUp(self) -> None:
        self.sim = Experiment_v1(
            geometry_dir='examples/Example_01/meshes/',
            model_name="mesh_2d",
        ...
        noise_level = 2.0
        inputs = [{'Qc': 100, 'T_amb': 20, 'atol': 1e-7, 'rtol': 1e-7}]*2
        probes = self.sim.solve_steady(**inputs[0])
        outputs = [
            probes.get_value('T') + noise_level*np.ones_like(probes.get_value('T')),
            probes.get_value('T') - noise_level*np.ones_like(probes.get_value('T'))]
        self.fitter = SteadyStateComparer(self.sim, inputs, outputs)
        self.m_sand = 5 # sand material
        self.m_contact = 6 # contact material
    ...
    def minimisation_test_recipe(self, m):
        ...
        print(f"true_k: {true_k}")
        k = true_k.copy() * 1.1 # initial guess -> make it 10% wrong
        opt = optimizers.QuasiNewtonBFGS(grad)
        sol_k = opt.optimise(k, max_iter=100)
        self.assertTrue(np.isclose(true_k, sol_k, atol=5e-4).all(), 
                        f"Values do not agree {true_k} vs {sol_k}")
