...
def build_geometry(
    ...
    spec = inspect.getargspec(build_geometry).args
    local_scope = locals()
    ...
    inner_cylinder = gmsh.model.occ.addCylinder(0, 0, t_insulation, 0, 0, h_sand, d_sand/2, angle=angle/2)
    if symmetry:
    ...
    for j in range(n_c):
        ...
        cartridges.append(c)
        spreaders.append(s)
        cartridge_tc.append([x, y, t_insulation+h_sand-0.5*h_c])
    ...
    d_m_angle = (2*math.asin((d_m/2)/(d_sand/2+dr_m)))
    n_m_request = (math.pi/n_c*n_m_ratio)/d_m_angle
    if n_m_request <= 0.75:
        n_m = 1
    elif n_m_request <= 1.5:
        n_m = 2
    else:
        n_m = int(n_m_request) + 1

    # cut holes for membrane heating
    holes = []
    m_phi = angle/2/max(1, n_m-1)
    for j in range(n_m):
        x, y = (d_sand/2+dr_m)*math.cos(-j*m_phi), (d_sand/2+dr_m)*math.sin(-j*m_phi)
    ...
    gmsh.model.mesh.field.setString(3, "F", f"{mesh_size_min}+{mesh_growth_factor}*F1")
