...
class HallC3(Term):
    ...
    def define_terms(self):
        ...
        self.T_room_ctrl_interval = (0.1, 0.2)
        self.converge_tol_T_room = 0.1
        self.converge_tol_Q_amb = 10
        ...
        self.area_m_total = sum(self.sim.get_surface_area(f'THP_surface_{i}_{j}') for i in range(n_mem_pipes) for j in range(n_thp_surface_segments))
        ...
        Q_m_updates = [[]]*n_mem_pipes
        for i in range(n_mem_pipes):
            for j in range(n_thp_surface_segments):
                self.new_constant(f'Q_m_{i}_{j}', 0.0)
                Q_m_updates[i].append(self.update_from_form_integral(f'THP_surface_{i}_{j}'))
                self.set_update(f'Q_m_{i}_{j}', Q_m_updates[i][j], prevent_override=True)

        self.new_constant('Q', 0.0)
        all_qm = [self.get_current_value(f'Q_m_{i}_{j}') for i in range(n_mem_pipes) for j in range(n_thp_surface_segments)]
        self.set_update('Q', lambda t: self.get_current_value('Q_s') + sum(all_qm), prevent_override=True)
...
class C3_passive(Simulation):
    ...
    def create_common_probes(self, probes: Probe_writer):
        ...
        # Heat flow from mem pipes surfaces of the storage to the room
        n_mem_pipes = self.get_custom_data('n_mem_pipes')
        n_thp_surface_segments = self.get_custom_data('n_thp_surface_segments')
        for i in range(n_mem_pipes):
            for j in range(n_thp_surface_segments):
        ...
        # Total heat flow from mem pipes to the room
        ...
        # Mean mixed tempearture of the mem pipes
    ...
    def solve_unsteady(self,
            ...
            max_mem_power=30000,
            datetime_start='2007-6-1 00:00:00',
        ...
        heating_season_t = meteodata.to_function(meteo_data, 'heating_season', offset_date=datetime_start, interpolate=False)
        heating_pause_t = meteodata.to_function(meteo_data, 'heating_pause', offset_date=datetime_start, interpolate=False)
