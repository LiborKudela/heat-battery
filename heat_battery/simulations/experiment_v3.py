from .simulation_base import Simulation, fem, np, MPI
...
class Experiment_v3(Simulation):
    def define_form_subdomain_terms(self):

        unsteady_wire_heating = unsteady_terms.SerialWiresResistiveHeating(self)

        # change update function in the wire
        unsteady_wire_heating.Q = 1.0
        def constQ_update(t):
            if t is self.t:
                unsteady_wire_heating.current.value[0] = np.sqrt(unsteady_wire_heating.Q/self.probes.get_value('R')*1000)
            elif t is self.t_n:
                unsteady_wire_heating.current.value[1] = np.sqrt(unsteady_wire_heating.Q/self.probes.get_value('R')*1000)
        unsteady_wire_heating.update = constQ_update

        self.set_unsteady_source_term(unsteady_wire_heating, 'long_wire')
        self.set_unsteady_source_term(unsteady_wire_heating, 'short_wire')

        steady_wire_heating = steady_terms.SerialWiresResistiveHeating(self)
        self.set_steady_state_source_term(steady_wire_heating, 'long_wire')
        self.set_steady_state_source_term(steady_wire_heating, 'short_wire')
    ...
    def set_unsteady_current(self, value):
        obj = self.get_unsteady_source_term('long_wire')
    ...
    def create_probes(self, probes):
        ...
        obj = self.get_unsteady_source_term('short_wire')
        ...
        long_power_form = fem.form(obj(self.T, self.t, self.x, 'long_wire')*self.jac*self.get_measure_dx('long_wire'))
        ...
        short_power_form = fem.form(obj(self.T, self.t, self.x, 'short_wire')*self.jac*self.get_measure_dx('short_wire'))
        ...
        def evaluate():
            return self.probes.get_value('total_power')/total_wire_l
    ...
    def run_experiment(self, res_path="results/probes_Q_const.csv", 
        ...
        self.get_unsteady_source_term('long_wire').Q = P
        ...
        self.imag_R0 = self.probes.get_value('imag_R')
