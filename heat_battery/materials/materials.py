...
def new_contact_class(d, name="New contact"):
    class Contact_class(Material):
        def __init__(self, domain, name=name):
            super().__init__(
                h0_T_ref = 20,
                k = Lagrange_property(domain, [20, 1000], [0.1, 0.1], PropertyUnits.K, multiplier=d*1000),
                rho = Lagrange_property(domain, [20], [1], PropertyUnits.rho),
                cp = Lagrange_property(domain, [20], [1], PropertyUnits.cp), 
                name=name)
